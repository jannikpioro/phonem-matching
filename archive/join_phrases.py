...
def main():
    ...
    for verb in set(noun_verb_by_verb) & set(adv_verb_by_verb):
        for nv in noun_verb_by_verb[verb]:
            for advv in adv_verb_by_verb[verb]:
                adv = advv['english_phrase'].split()[1]
                english_phrase = f"{nv['english_phrase'].split()[0]} {advv['english_phrase']}"
                ger_adv = advv['german_phrase'].split()[1]
                german_phrase = f"{nv['german_phrase'].split()[0]} {advv['german_phrase']}"
                english_phonetic = f"{nv['english_phonetic'].split()[0]} {advv['english_phonetic']}"
                german_phonetic = f"{nv['german_phonetic'].split()[0]} {advv['german_phonetic']}"
    ...
    # 4-word: adj noun adv verb
    for noun in set(adj_noun_by_noun) & set(noun_verb_by_noun):
        for verb in set(noun_verb_by_verb) & set(adv_verb_by_verb):
            for nv in [p for p in noun_verb_by_noun[noun] if p['english_phrase'].split()[1] == verb]:
                for adjn in adj_noun_by_noun[noun]:
                    for advv in adv_verb_by_verb[verb]:
                        english_phrase = f"{adjn['english_phrase']} {advv['english_phrase']}"
                        german_phrase = f"{adjn['german_phrase']} {advv['german_phrase']}"
                        english_phonetic = f"{adjn['english_phonetic']} {advv['english_phonetic']}"
                        german_phonetic = f"{adjn['german_phonetic']} {advv['german_phonetic']}"
                        phonetic_sim = (adjn['adjective_phonetic_similarity'] + adjn['noun_phonetic_similarity'] + nv['verb_phonetic_similarity'] + advv['adverb_phonetic_similarity']) / 4
                        semantic_sim = phrase_similarity(english_phrase)
                        results_4.append({
                            'english_phrase': english_phrase,
                            'german_phrase': german_phrase,
